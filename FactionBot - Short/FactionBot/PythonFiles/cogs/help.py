# -*- coding: utf-8 -*-
"""cogs/help.py — dynamic FactionBot help system.
...
at startup), so it keeps working as cogs are added or removed. Pages are
ephemeral when invoked via slash and regular messages when invoked via
prefix. Components are gated to the invoking user and time out after 180s.
"""
...
try:  # normal case: loaded as part of the ``cogs`` package
    from . import botkit
except ImportError:  # pragma: no cover — loaded as a top-level module
    import botkit  # type: ignore[no-redef]
...
class HelpCog(commands.Cog, name="FactionHelp"):
    ...
    def _perm_hints(cls, command: commands.Command) -> List[str]:
        ...
        app_command = getattr(command, "app_command", None)
        if getattr(app_command, "guild_only", False):
            hints.add("Server only (no DMs)")
        perms = getattr(app_command, "default_permissions", None)
        if perms is not None:
            try:
                perm_names = [n.replace("_", " ").title() for n, v in perms if v]
                if perm_names:
                    hints.add(f"Slash default permissions: {', '.join(perm_names[:5])}")
            except Exception:
                pass
        return sorted(hints)
    ...
    def _command_embed(self, command: commands.Command) -> discord.Embed:
        ...
        if isinstance(command, (commands.HybridCommand, commands.HybridGroup)):
            app_command = getattr(command, "app_command", None)
            app_name = getattr(app_command, "qualified_name", None) or qualified
            parts.append(f"*Also available as a slash command: `/{app_name}`*")
        if isinstance(command, commands.Group):
