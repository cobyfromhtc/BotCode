...
All persistence goes through :mod:`cogs.botkit` (same SQLite file as the core
bot, WAL mode). This module never imports from Bot.py.
"""
...
try:  # loaded as a package extension (cogs.invites) — preferred
    from . import botkit as bk
...
def _require_manage_guild():
    """commands.check gate for Manage Server (works for prefix + slash)."""
...
class InviteTrackingCog(commands.Cog, name="InviteTracking"):
    ...
    # ------------------------------------------------------------------
    @commands.hybrid_group(name="invites", aliases=["invite"], invoke_without_command=True)
    @commands.guild_only()
    ...
    @commands.hybrid_command(name="isetup")
    @commands.guild_only()
    @_require_manage_guild()
    @app_commands.default_permissions(manage_guild=True)
